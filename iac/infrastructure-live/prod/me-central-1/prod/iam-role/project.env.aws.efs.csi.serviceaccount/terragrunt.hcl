...
dependencies {
  paths = ["../../iam-policy/project.env.aws.efs.csi.serviceaccount",
  "../../eks/project-env-eks"]
}

dependency "autoscaler" {
  config_path = "../../iam-policy/project.env.aws.efs.csi.serviceaccount"
  mock_outputs = {
...
inputs = {
  ...
  description                   = "Allows allows the CSI driver's service account to make calls to AWS APIs in EKS Cluster"
  create_role                   = true
